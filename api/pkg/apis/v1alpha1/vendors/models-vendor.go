...
func (c *ModelsVendor) onModels(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	defer span.End()
	tLog.WithContext(pCtx).Debugf("V (Models): onModels, method: %s", request.Method)
	...
	case fasthttp.MethodGet:
		...
		if err != nil {
			if isArray {
				tLog.WithContext(ctx).Errorf(" V (Models): onModels failed to ListSpec, err: %v", err)
			} else {
				tLog.WithContext(ctx).Errorf(" V (Models): onModels failed to GetSpec, id: %s, err: %v", id, err)
			}
	...
	case fasthttp.MethodPost:
		...
		if err != nil {
			tLog.WithContext(ctx).Errorf("V (Models): onModels failed to pause model from request body, error: %v", err)
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
		...
		if err != nil {
			tLog.WithContext(ctx).Errorf("V (Models): onModels failed to UpsertSpec, id: %s, error: %v", id, err)
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
	...
	case fasthttp.MethodDelete:
		...
		if err != nil {
			tLog.WithContext(ctx).Errorf("V (Models): onModels failed to DeleteSpec, id: %s, error: %v", id, err)
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
	...
	tLog.WithContext(pCtx).Error("V (Models): onModels returned MethodNotAllowed")
