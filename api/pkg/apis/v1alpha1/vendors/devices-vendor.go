...
func (c *DevicesVendor) onDevices(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	defer span.End()
	tLog.WithContext(pCtx).Infof("V (Devices): onDevices %s", request.Method)
	...
	case fasthttp.MethodGet:
		...
		if err != nil {
			log.WithContext(ctx).Errorf("V (Devices): failed to get device spec, error %v", err)
	...
	case fasthttp.MethodPost:
		...
		if err != nil {
			log.WithContext(ctx).Errorf("V (Devices): failed to unmarshall request body, error %v", err)
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
		...
		if err != nil {
			log.WithContext(ctx).Errorf("V (Devices): failed to upsert device spec, error %v", err)
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
	...
	case fasthttp.MethodDelete:
		...
		if err != nil {
			log.WithContext(ctx).Errorf("V (Devices): failed to delete device spec, error %v", err)
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
	...
	log.WithContext(pCtx).Error("V (Devices): onDevices returns MethodNotAllowed")
