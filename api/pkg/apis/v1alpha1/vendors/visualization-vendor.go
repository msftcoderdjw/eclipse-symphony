...
func (c *VisualizationVendor) onVisPacket(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	defer span.End()
	vcLog.WithContext(pCtx).Info("V (Models): onVisPacket")
	...
	case fasthttp.MethodPost:
		...
		if err != nil {
			vcLog.WithContext(pCtx).Errorf("V (Visualization): onVisPacket failed - %s", err.Error())
		...
		if !packet.IsValid() {
			vcLog.WithContext(pCtx).Errorf("V (Visualization): onVisPacket failed - %s", "invalid visualization packet")
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.BadRequest,
