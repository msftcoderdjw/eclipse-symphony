...
func (c *InstancesVendor) onInstances(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	defer span.End()
	iLog.WithContext(pCtx).Infof("V (Instances): onInstances, method: %s", request.Method)
	...
	iLog.WithContext(pCtx).Infof("V (Instances): onInstances, method: %s", string(request.Method))
	...
	case fasthttp.MethodGet:
		...
		if err != nil {
			iLog.WithContext(ctx).Errorf("V (Instances): onInstances failed - %s", err.Error())
	...
	case fasthttp.MethodPost:
		...
		if solution != "" && (target != "" || target_selector != "") {
			...
			if target != "" {
				...
				if len(parts) != 2 {
					iLog.WithContext(ctx).Error("V (Instances): onInstances failed - invalid target selector format")
					return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
						State: v1alpha2.InternalError,
	...
	iLog.WithContext(pCtx).Info("V (Instances): onInstances failed - 405 method not allowed")
