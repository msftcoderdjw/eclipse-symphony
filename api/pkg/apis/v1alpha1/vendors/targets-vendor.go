...
func (c *TargetsVendor) onRegistry(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	defer span.End()
	tLog.WithContext(pCtx).Infof("V (Targets) : onRegistry, method: %s", request.Method)
	...
	case fasthttp.MethodGet:
		...
		if err != nil {
			tLog.WithContext(ctx).Errorf("V (Targets) : onRegistry failed - %s", err.Error())
	...
	case fasthttp.MethodPost:
		...
		if binding != "" {
			if binding == "staging" {
				...
				tLog.WithContext(ctx).Error("V (Targets) : onRegistry failed - invalid binding")
				return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
	...
	tLog.WithContext(pCtx).Error("V (Targets) : onRegistry failed - method not allowed")
...
func (c *TargetsVendor) onBootstrap(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	defer span.End()
	tLog.WithContext(ctx).Infof("V (Targets) : onBootstrap, method: %s", request.Method)
	switch request.Method {
	case fasthttp.MethodPost:
		var authRequest AuthRequest
		err := json.Unmarshal(request.Body, &authRequest)
		if err != nil || authRequest.UserName != "symphony-test" {
			tLog.WithContext(ctx).Errorf("V (Targets) : onBootstrap failed - %s", err.Error())
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
	...
	tLog.WithContext(ctx).Error("V (Targets) : onRegistry failed - method not allowed")
	resp := v1alpha2.COAResponse{
		State:       v1alpha2.MethodNotAllowed,
		Body:        []byte("{\"result\":\"405 - method not allowed\"}"),
		ContentType: "application/json",
...
func (c *TargetsVendor) onStatus(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	defer span.End()
	tLog.WithContext(pCtx).Infof("V (Targets) : onStatus, method: %s", request.Method)
	...
	case fasthttp.MethodPut:
		...
		if err != nil {
			tLog.WithContext(pCtx).Errorf("V (Targets) : onStatus failed - %s", err.Error())
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
	...
	tLog.WithContext(pCtx).Error("V (Targets) : onStatus failed - method not allowed")
	resp := v1alpha2.COAResponse{
		State:       v1alpha2.MethodNotAllowed,
		Body:        []byte("{\"result\":\"405 - method not allowed\"}"),
		ContentType: "application/json",
...
func (c *TargetsVendor) onDownload(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	defer span.End()
	tLog.WithContext(pCtx).Infof("V (Targets) : onDownload, method: %s", request.Method)
	...
	case fasthttp.MethodGet:
		...
		if err != nil {
			tLog.WithContext(pCtx).Errorf("V (Targets) : onDownload failed - %s", err.Error())
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
	...
	tLog.WithContext(pCtx).Error("V (Targets) : onDownload failed - method not allowed")
	resp := v1alpha2.COAResponse{
		State:       v1alpha2.MethodNotAllowed,
		Body:        []byte("{\"result\":\"405 - method not allowed\"}"),
		ContentType: "application/json",
...
func (c *TargetsVendor) onHeartBeat(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	defer span.End()
	tLog.WithContext(pCtx).Infof("V (Targets) : onHeartBeat, method: %s", request.Method)
	...
	case fasthttp.MethodPost:
		...
		if err != nil {
			tLog.WithContext(pCtx).Errorf("V (Targets) : onHeartBeat failed - %s", err.Error())
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
	...
	tLog.WithContext(pCtx).Error("V (Targets) : onHeartBeat failed - method not allowed")
	resp := v1alpha2.COAResponse{
		State:       v1alpha2.MethodNotAllowed,
		Body:        []byte("{\"result\":\"405 - method not allowed\"}"),
		ContentType: "application/json",
