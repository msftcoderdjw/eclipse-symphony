...
func (c *ActivationsVendor) onStatus(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	vLog.WithContext(pCtx).Infof("V (Activations Vendor): onStatus, method: %s", string(request.Method))
	...
	case fasthttp.MethodPost:
		...
		if err != nil {
			vLog.WithContext(ctx).Infof("V (Activations Vendor): onStatus failed - %s", err.Error())
	...
	vLog.WithContext(pCtx).Info("V (Activations Vendor): onStatus failed - 405 method not allowed")
...
func (c *ActivationsVendor) onActivations(request v1alpha2.COARequest) v1alpha2.COAResponse {
	...
	vLog.WithContext(pCtx).Infof("V (Activations Vendor): onActivations, method: %s", string(request.Method))
	...
	case fasthttp.MethodGet:
		...
		if err != nil {
			vLog.WithContext(ctx).Infof("V (Activations Vendor): onActivations failed - %s", err.Error())
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
	...
	case fasthttp.MethodPost:
		...
		if err != nil {
			vLog.WithContext(ctx).Errorf("V (Activations Vendor): onActivations failed - %s", err.Error())
			return observ_utils.CloseSpanWithCOAResponse(span, v1alpha2.COAResponse{
				State: v1alpha2.InternalError,
				Body:  []byte(err.Error()),
			})
	...
	vLog.WithContext(pCtx).Info("V (Activations Vendor): onActivations failed - 405 method not allowed")
	resp := v1alpha2.COAResponse{
		State:       v1alpha2.MethodNotAllowed,
		Body:        []byte("{\"result\":\"405 - method not allowed\"}"),
		ContentType: "application/json",
