...
func (i *DockerTargetProvider) Get(ctx context.Context, deployment model.DeploymentSpec, references []model.ComponentStep) ([]model.ComponentSpec, error) {
	...
	sLog.WithContext(ctx).Infof("  P (Docker Target): getting artifacts: %s - %s", deployment.Instance.Spec.Scope, deployment.Instance.ObjectMeta.Name)

	cli, err := client.NewClientWithOpts(client.FromEnv)
	if err != nil {
		sLog.WithContext(ctx).Errorf("  P (Docker Target): failed to create docker client: %+v", err)
		return nil, err
	}
...
func (i *DockerTargetProvider) Apply(ctx context.Context, deployment model.DeploymentSpec, step model.DeploymentStep, isDryRun bool) (map[string]model.ComponentResultSpec, error) {
	...
	sLog.WithContext(ctx).Infof("  P (Docker Target): applying artifacts: %s - %s", deployment.Instance.Spec.Scope, deployment.Instance.ObjectMeta.Name)
	...
	if err != nil {
		sLog.WithContext(ctx).Errorf("  P (Docker Target): failed to validate components: %+v", err)
		return nil, err
	}
	if isDryRun {
		err = nil
	...
	for _, component := range step.Components {
		if component.Action == model.ComponentUpdate {
			...
			if image == "" {
				...
				sLog.WithContext(ctx).Errorf("  P (Docker Target): %+v", err)
				return ret, err
			}
			...
			if err != nil {
				...
				sLog.WithContext(ctx).Errorf("  P (Docker Target): failed to pull docker image: %+v", err)
				return ret, err
			}
			...
			if alreadyRunning {
				...
				if err != nil {
					if !client.IsErrNotFound(err) {
						sLog.WithContext(ctx).Errorf("  P (Docker Target): failed to stop a running container: %+v", err)
						return ret, err
					}
				}
				...
				if err != nil {
					...
					sLog.WithContext(ctx).Errorf("  P (Docker Target): failed to remove existing container: %+v", err)
					return ret, err
				}
			}
			...
			if resources != "" {
				...
				if err != nil {
					...
					sLog.WithContext(ctx).Errorf("  P (Docker Target): failed to read container resource settings: %+v", err)
					return ret, err
				}
			...
			if err != nil {
				...
				sLog.WithContext(ctx).Errorf("  P (Docker Target): failed to create container: %+v", err)
				return ret, err
			}
			...
			if err = cli.ContainerStart(context.TODO(), container.ID, types.ContainerStartOptions{}); err != nil {
				...
				sLog.WithContext(ctx).Errorf("  P (Docker Target): failed to start container: %+v", err)
				return ret, err
			}
			ret[component.Component.Name] = model.ComponentResultSpec{
