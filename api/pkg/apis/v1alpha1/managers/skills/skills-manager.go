...
func (t *SkillsManager) DeleteState(ctx context.Context, name string, namespace string) error {
	...
	log.WithContext(ctx).Debugf(" M (Skills): DeleteState, name: %s", name)
	...
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Skills): failed to delete state, name: %s, err: %v", name, err)
	}
	return err
}
...
func (t *SkillsManager) UpsertState(ctx context.Context, name string, state model.SkillState) error {
	...
	var err error = nil
	defer observ_utils.CloseSpanWithError(span, &err)
	log.WithContext(ctx).Debugf(" M (Skills): UpsertState, name: %s", name)
	...
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Skills): failed to UpsertSpec, name: %s, err: %v", name, err)
		return err
	}
	return nil
}
...
func (t *SkillsManager) ListState(ctx context.Context, namespace string) ([]model.SkillState, error) {
	...
	log.WithContext(ctx).Debug(" M (Skills): ListState")
	...
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Skills): failed to list state, err: %v", err)
		return nil, err
	}
	...
	for _, t := range models {
		...
		if err != nil {
			log.WithContext(ctx).Errorf(" M (Models): failed to get skill state, err: %v", err)
			return nil, err
		}
		ret = append(ret, rt)
	}
...
func (t *SkillsManager) GetState(ctx context.Context, name string, namespace string) (model.SkillState, error) {
	...
	log.WithContext(ctx).Debugf(" M (Skills): GetState, name: %s", name)
	...
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Skills): failed to get state, name: %s, err: %v", name, err)
		return model.SkillState{}, err
	}
	var ret model.SkillState
	ret, err = getSkillState(m.Body)
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Skills): failed to get skill state, name: %s, err: %v", name, err)
		return model.SkillState{}, err
	}
	return ret, nil
}
