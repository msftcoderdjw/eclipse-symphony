...
func (t *ModelsManager) DeleteState(ctx context.Context, name string, namespace string) error {
	...
	log.WithContext(ctx).Debugf(" M (Models): DeleteState, name: %s", name)
	...
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Models): failed to delete state, name: %s, err: %v", name, err)
	}
	return err
}
...
func (t *ModelsManager) UpsertState(ctx context.Context, name string, state model.ModelState) error {
	...
	var err error = nil
	defer observ_utils.CloseSpanWithError(span, &err)
	log.WithContext(ctx).Debugf(" M (Models): UpsertState, name: %s", name)
	...
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Models): failed to UpsertSpec, name: %s, err: %v", name, err)
		return err
	}
	return nil
}
...
func (t *ModelsManager) ListState(ctx context.Context, namespace string) ([]model.ModelState, error) {
	...
	log.WithContext(ctx).Debug(" M (Models): ListState")
	...
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Models): failed to ListState, err: %v", err)
		return nil, err
	}
	...
	for _, t := range models {
		...
		if err != nil {
			log.WithContext(ctx).Errorf(" M (Models): failed to getModelState, err: %v", err)
			return nil, err
		}
		ret = append(ret, rt)
	}
...
func (t *ModelsManager) GetState(ctx context.Context, name string, namespace string) (model.ModelState, error) {
	...
	log.WithContext(ctx).Debugf(" M (Models): GetState, name: %s", name)
	...
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Models): failed to GetSpec, name: %s, err: %v", name, err)
		return model.ModelState{}, err
	}
	...
	if err != nil {
		log.WithContext(ctx).Errorf(" M (Models): failed to getModelState, name: %s, err: %v", name, err)
		return model.ModelState{}, err
	}
	return ret, nil
}
