...
func (t *UsersManager) DeleteUser(ctx context.Context, name string) error {
	...
	log.WithContext(ctx).Infof(" M (Users): DeleteUser name %s", name)

	err = t.StateProvider.Delete(ctx, states.DeleteRequest{
		ID: name,
	})
	if err != nil {
		log.WithContext(ctx).Debugf(" M (Users) : failed to delete user %s", err)
		return err
	}
	return nil
}
...
func (t *UsersManager) UpsertUser(ctx context.Context, name string, password string, roles []string) error {
	...
	var err error = nil
	defer observ_utils.CloseSpanWithError(span, &err)
	log.WithContext(ctx).Infof(" M (Users): UpsertUser name %s", name)
	...
	if err != nil {
		log.WithContext(ctx).Debugf(" M (Users) : failed to upsert user %v", err)
		return err
	}
	return nil
}
func (t *UsersManager) CheckUser(ctx context.Context, name string, password string) ([]string, bool) {
	...
	var err error = nil
	defer observ_utils.CloseSpanWithError(span, &err)
	log.WithContext(ctx).Infof(" M (Users): CheckUser name %s", name)
	...
	if err != nil {
		log.WithContext(ctx).Debugf(" M (Users) : failed to get user %s states", err)
		return nil, false
	}
	...
	if hash(name, password) == userState.PasswordHash {
		log.WithContext(ctx).Debug(" M (Users) : user authenticated")
		return userState.Roles, true
	}

	log.WithContext(ctx).Debug(" M (Users) : authentication failed")
	return nil, false
}
