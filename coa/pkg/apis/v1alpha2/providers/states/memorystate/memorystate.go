...
func (s *MemoryStateProvider) Upsert(ctx context.Context, entry states.UpsertRequest) (string, error) {
	...
	sLog.WithContext(ctx).Debugf("  P (Memory State): upsert states %s in namespace %s", entry.Value.ID)
	...
	if !ok {
		...
		sLog.WithContext(ctx).Errorf("  P (Memory State): failed to upsert %s states: %+v", entry.Value.ID, err)
		return "", err
	}
	if entry.Options.UpdateStateOnly {
		...
		if !ok {
			...
			sLog.WithContext(ctx).Errorf("  P (Memory State): failed to upsert %s state: %+v", entry.Value.ID, err)
			return "", err
		}
...
func (s *MemoryStateProvider) List(ctx context.Context, request states.ListRequest) ([]states.StateEntry, string, error) {
	...
	sLog.WithContext(ctx).Debugf("  P (Memory State): list states in namespace %s", namespace)
					...
					if ok {
						if request.FilterType != "" && request.FilterValue != "" {
							...
							if err != nil {
								...
								sLog.WithContext(ctx).Errorf("  P (Memory State): failed to list states: %+v", err)
								return entities, "", err
							}
...
func (s *MemoryStateProvider) Delete(ctx context.Context, request states.DeleteRequest) error {
	...
	sLog.WithContext(ctx).Debugf("  P (Memory State): delete state %s in namespace %s", request.ID, namespace)

	if _, ok := s.Data[namespace]; !ok {
		err = v1alpha2.NewCOAError(nil, fmt.Sprintf("entry '%s' is not found in namespace %s", request.ID, namespace), v1alpha2.NotFound)
		sLog.WithContext(ctx).Errorf("  P (Memory State): failed to delete %s: %+v", request.ID, err)
		return err
	}
	list, ok := s.Data[namespace].(map[string]interface{})
	if !ok {
...
func (s *MemoryStateProvider) Get(ctx context.Context, request states.GetRequest) (states.StateEntry, error) {
	...
	sLog.WithContext(ctx).Debugf("  P (Memory State): get state %s in namespace %s", request.ID, namespace)

	if _, ok := s.Data[namespace]; !ok {
		err = v1alpha2.NewCOAError(nil, fmt.Sprintf("entry '%s' is not found in namespace %s", request.ID, namespace), v1alpha2.NotFound)
		sLog.WithContext(ctx).Errorf("  P (Memory State): failed to get %s state: %+v", request.ID, err)
