...
func testHttpRequestHelperWithHeaders(t *testing.T, method string, url string, body []byte, headers map[string]string, expectedStatusCode int, expectedBody string, expectedResponseHeaders map[string]string) {
	client := &http.Client{}
	req, err := http.NewRequest(method, url, bytes.NewBuffer(body))
	assert.Nil(t, err)
...
func TestHTTPEcho(t *testing.T) {
	...
	testHttpRequestHelper(t, fasthttp.MethodGet, "http://localhost:8080/v1/greetings", nil, 200, "Hi there!!")

	// query args
	testHttpRequestHelper(t, fasthttp.MethodGet, "http://localhost:8080/v1/greetings2?name=John", nil, 200, "Hi John!!")

	// path parameters
	testHttpRequestHelper(t, fasthttp.MethodGet, "http://localhost:8080/v1/greetings3/John", nil, 200, "Hi John!!!")
...
func TestHTTEchoWithPipeline(t *testing.T) {
	...
	testHttpRequestHelperWithHeaders(t, fasthttp.MethodGet, "http://localhost:8081/v1/greetings", nil,
		map[string]string{
	...
	// invalid token
	testHttpRequestHelperWithHeaders(t, fasthttp.MethodGet, "http://localhost:8081/v1/greetings", nil, map[string]string{
		"Authorization": "Bearer fake-token",
	}, 403, "", nil)

	// empty token
	testHttpRequestHelperWithHeaders(t, fasthttp.MethodGet, "http://localhost:8081/v1/greetings", nil, nil, 403, "", nil)

	// test valid token and wrong rbac (method not match)
	testHttpRequestHelperWithHeaders(t, fasthttp.MethodPost, "http://localhost:8081/v1/greetings", []byte("John"), map[string]string{
		"Authorization": "Bearer " + authHeader,
	}, 403, "", nil)
