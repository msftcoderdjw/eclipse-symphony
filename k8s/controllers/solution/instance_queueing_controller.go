...
func (r *InstanceQueueingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if checkSkipReconcile(log, ctx, instance) {
		diagnostic.InfoWithCtx(log, ctx, "Skipping this reconcile, since this instance is inactive and already removed")
		return ctrl.Result{}, nil
	}
...
func checkSkipReconcile(log logr.Logger, ctx context.Context, instance *solution_v1.Instance) bool {
	...
	if instance.Status.Properties != nil {
		...
		if !ok || status != string(utilsmodel.ProvisioningStatusSucceeded) {
			diagnostic.InfoWithCtx(log, ctx, "Instance has not reach succeeded status, do not skip reconcile")
			return false
		}
		removed, ok := instance.Status.Properties["removed"]
		if !ok || removed != "true" {
			diagnostic.InfoWithCtx(log, ctx, "Instance has not been removed, do not skip reconcile")
			return false
		}
		diagnostic.InfoWithCtx(log, ctx, "Instance is inactive and already removed, skip reconcile")
		return true
	}
	diagnostic.InfoWithCtx(log, ctx, "Instance status is nil, do not skip reconcile")
	return false
}
